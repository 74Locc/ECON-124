...
model <- glm(HEARD ~ is_Placebo, data = fake_plac, family = 'binomial')
summary(model)

#b.)
survey$ThoughtTrue <- factor(survey$ThoughtTrue)
levels(survey$ThoughtTrue)

model2 <- glm(HEARD ~ ThoughtTrue, data = fake_plac, family = 'binomial')
summary(model2)
